...
public class UpgradeKernelPackageTest extends UpgradeKernelPackage {
	...
	public void testPreventDuplicates() throws Exception {
		...
		_checkDataValid(
			"Counter", "name", _NEW_RESOURCE_NAME, WildcardMode.SURROUND, false,
			false);
	}

	@Test
	public void testUpgradeWithoutMatchData() throws Exception {
		_checkDataValid("ListType", "type_", _OLD_RESOURCE_NAME, false, false);
		upgradeTable(
			"ListType", "type_", getClassNames(), WildcardMode.SURROUND);
		_checkDataValid("ListType", "type_", _NEW_RESOURCE_NAME, false, false);
	}
	...
	private void _checkDataValid(
			String tableName, String columnName, String value,
			boolean ifLongtext, boolean expected)
		throws Exception {

		_checkDataValid(
			tableName, columnName, value, null, ifLongtext, expected);
	}
	...
	private void _checkUpgrade() throws Exception {
		...
		_checkDataValid(
			...
			false, false);
		_checkDataValid("Counter", "name", postfixNewClassName, false, true);
		_checkDataValid(
			"Lock_", "className", surroundNewClassName, false, true);
		_checkDataValid(
			"ResourceAction", "name", prefixNewClassName, false, true);
		_checkDataValid(
			"ResourceBlock", "name", postfixNewClassName, false, true);
		_checkDataValid(
			"ResourcePermission", "name", surroundNewClassName, false, true);
		_checkDataValid(
			"UserNotificationEvent", "payload", prefixNewClassName, true, true);
		_checkDataValid("ListType", "type_", prefixNewClassName, false, false);
		_checkDataValid(
			"ResourceAction", "name", postfixNewResourceName, false, false);
		_checkDataValid(
			"ResourceBlock", "name", prefixNewResourceName, false, true);
		_checkDataValid(
	...
	private void _dataPreparation() throws Exception {
		...
		_insertTableValues(
			...
			String.valueOf(increment(ResourcePermission.class)),
			String.valueOf(TestPropsValues.getCompanyId()),
			surroundOldResourceName, "4", "'PRIM_KEY2'", "2", "3", "4", "5",
			"[$TRUE$]");
